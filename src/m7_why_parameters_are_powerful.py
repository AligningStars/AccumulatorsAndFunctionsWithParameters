...
def run_test_even_better_draw_circles():
    test_even_better_draw_circles(20, 7, 300, 300, 'green', 0.25)


def test_even_better_draw_circles(t, n, s1, s2, c, r):
    window = rg.RoseWindow(600, 600)

    center = rg.Point(s1, s2)
    for k in range(n):
        circle = rg.Circle(center, t * k)
        circle.outline_color = c
        circle.attach_to(window)
        window.render(r)  # Pauses for r seconds after rendering.
